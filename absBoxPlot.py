...
class Boxplot(Canvas):
    ...
    def draw(self):
        ...
        for n in range(_VERTICAL_NODES):
            ...
            # text
            self.create_text(start_x - 10, (start_y + end_y) / 2,
                             text=str(vertical_value_step_size * (_VERTICAL_NODES - n )),
                             font=("Arial", 9, "bold"))

        # draw rectangles for every value
        ...
        for k in self.sorted_data:
            ...
            self.create_rectangle(x - 5, start_y, x + 5, start_y - 10, fill="black")
            self.create_text(x + 25, start_y, text=str(highest_value))
            ...
            # convert quartils to coordinates
...
if __name__ == '__main__':
    w = Tk()
    d: dict = {"1": [22, 31, 33, 3, 17, 16, 1, 2, 2, 3], "2": [1, 2, 3, 4, 5, 6, 7, 8]}
